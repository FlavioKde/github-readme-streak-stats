...
const diffDays =
  (block.to.getTime() - block.from.getTime()) /
  (1000 * 60 * 60 * 24);

console.log('DAYS SPAN:', diffDays);
...
console.log('DAYS SPAN:', diffDays);
  ...
  console.log('BLOCK RANGE:', block.from, block.to);
...
/*
const from = new Date('2023-11-03T00:00:00Z');
const to = new Date('2024-11-02T23:59:59Z');

return await fetchSingleRange({ username }, from, to);
...
export async function buildYearBlocks(createdAt, currentDate = new Date()) {
  ...
  endDate.setUTCHours(0, 0, 0, 0); // Normalize to start of the day
  ...
  if(d1.getTime() === d2.getTime()) {
    bloks.push({ from: new Date(startDate), to: new Date(endDate) });
    return bloks;
  }
