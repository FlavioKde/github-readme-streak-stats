...
console.log('Today:', new Date().toISOString());
  ...
  for (const block of blocks) {
    ...
    //add debug logs to verify the data being fetched for each block
    console.log('Block:', block.from.toISOString(), '->', block.to.toISOString(), '| Days:', blockData.length);
...
export function buildYearBlocks(createdAt, currentDate = new Date()) {
  const blocks = [];
//añadido para debug 28-2
  const created = new Date(createdAt);
  created.setUTCHours(0, 0, 0, 0); // Asegura que la hora esté al inicio del día
  ...
  for (let year = startYear; year <= currentYear; year++) {
    ...
    */
    ...
    const from = year === startYear 
      ? new Date(created) 
      : new Date(Date.UTC(year, 0, 1)); // 1 de enero
